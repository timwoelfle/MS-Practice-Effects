...
if (params$test_code == "ips_over_ips_baseline") {
  ...
  print(ggplot(data, aes(ips, ips/ips_baseline, color=ips_baseline)) +
    geom_point(alpha=0.2) +
    geom_smooth(method="lm", se=T, color="black") +
    ggtitle(paste0("n=", nrow(data), " (", sum(data$ips/data$ips_baseline > 8), " outliers not shown), Pearson r=",round(cor(data$ips, data$ips/data$ips_baseline), 2), ", Spearman ρ=", round(cor(data$ips, data$ips/data$ips_baseline, method="spearman"), 2))) +
    ylim(0,8))
  
  data$ips = data$ips / data$ips_baseline # becomes value
  data$ips_baseline = NA # becomes hand_used
...
t(data.frame(
  ...
  percent_female = paste0(round(prop.table(table(participants_summary$sex == "female"))[[2]]*100, 1)),
...
p = ggplot() + geom_line(aes(repetition, value, group=id), data_censored, alpha=0.2, color="darkgrey") +
  ...
  geom_text(data=data.frame(intercept=coef(QR)[1,], label=paste0(percentiles*100, "th percentile: β = ", round(coef(QR)[2,],2), ", ", "p.adj ", sapply(p_vals,signif_p))),
            mapping=aes(x=min(data_censored$repetition)+0.2,y=intercept, label=label), color=linecolor, hjust="left", vjust=1, family="Serif")

if (exists("ANOVA")) {
  p + geom_text(aes(x=x,y=y,label=label), data.frame(x=0.7*params$min_repetitions, y=0, label=paste0("ANOVA p ", signif_p(ANOVA$table$pvalue, 1))), vjust=-1.5, family="Serif")
} else {
  p
}
```
